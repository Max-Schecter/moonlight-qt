...
APPLE_ARCHS="${APPLE_ARCHS:-$(uname -m)}"
...
pushd "$BUILD_FOLDER"
qmake "$SOURCE_ROOT/moonlight-qt.pro" QMAKE_APPLE_DEVICE_ARCHS="$APPLE_ARCHS" || fail "Qmake failed!"
popd
...
find "$BUILD_FOLDER/app/Moonlight.app/" -name '*.dSYM' -print0 | xargs -0 rm -rf
